...
import DevolucionVentaModal from '../../components/devolucion-venta/DevolucionVentaModal';
import { Colors } from '../../constants/colors';
import { Layout } from '../../constants/layout';
import { DevolucionVenta, deleteDevolucionVenta, getDevolucionesVenta } from '../../services/devolucionVentaService';

function estadoConfig(estado: string | null) {
  switch (estado) {
    case 'PENDIENTE':  return { label: 'Pendiente',  color: '#D97706', bg: '#FEF3C7', bar: '#F59E0B' };
    case 'APROBADA':   return { label: 'Aprobada',   color: '#059669', bg: '#ECFDF5', bar: '#10B981' };
    case 'RECHAZADA':  return { label: 'Rechazada',  color: '#DC2626', bg: '#FEF2F2', bar: '#EF4444' };
...
export default function DevolucionVentaScreen() {
  const [data, setData]         = useState<DevolucionVenta[]>([]);
  const [loading, setLoading]   = useState(true);
  const [search, setSearch]     = useState('');
  const [modalVisible, setModal]= useState(false);
  const [selected, setSelected] = useState<DevolucionVenta | null>(null);

  const fetch = async () => {
    setLoading(true);
    try { setData(await getDevolucionesVenta()); }
    catch { Toast.show({ type: 'error', text1: 'Error', text2: 'No se pudo cargar devoluciones.' }); }
    finally { setLoading(false); }
  };
  useEffect(() => { fetch(); }, []);

  const handleDelete = async (item: DevolucionVenta) => {
    try {
      await deleteDevolucionVenta(item.ID_DEVOLUCION_VENTA);
      Toast.show({ type: 'success', text1: 'Eliminado', text2: 'Devolución eliminada.' });
      fetch();
    } catch { Toast.show({ type: 'error', text1: 'Error', text2: 'No se pudo eliminar.' }); }
  };

  const filtered = data.filter(d =>
    (d.NUMERO_DEVOLUCION ?? '').toLowerCase().includes(search.toLowerCase()) ||
    (d.MOTIVO_DEVOLUCION ?? '').toLowerCase().includes(search.toLowerCase()) ||
    (d.ESTADO_DEVOLUCION_VENTA ?? '').toLowerCase().includes(search.toLowerCase())
  );
  ...
  return (
    <View style={st.container}>
      <View style={st.header}>
        <View>
          <Text style={st.pageTitle}>Devoluciones de Venta</Text>
      ...
      <View style={st.summaryRow}>
        <View style={st.summaryCard}>
          <FontAwesome5 name="undo-alt" size={13} color={Colors.primary} />
        ...
        <View style={st.summaryCard}>
          ...
          <Text style={[st.summaryValue, { color: '#D97706' }]}>{data.filter(d => d.ESTADO_DEVOLUCION_VENTA === 'PENDIENTE').length}</Text>
          <Text style={st.summaryLabel}>Pendientes</Text>
        </View>
        <View style={st.summaryCard}>
          <FontAwesome5 name="check-circle" size={13} color="#059669" />
          <Text style={[st.summaryValue, { color: '#059669' }]}>{data.filter(d => d.ESTADO_DEVOLUCION_VENTA === 'APROBADA').length}</Text>
          <Text style={st.summaryLabel}>Aprobadas</Text>
        </View>
      </View>

      <View style={st.searchWrapper}>
        <FontAwesome5 name="search" size={12} color={Colors.textMuted} />
        <TextInput style={st.searchInput} placeholder="Buscar por número, motivo o estado…" placeholderTextColor={Colors.textMuted} value={search} onChangeText={setSearch} />
      ...
      {loading ? (
        ...
        <View style={st.empty}><FontAwesome5 name="undo-alt" size={36} color={Colors.textMuted} /><Text style={st.emptyText}>Sin resultados</Text></View>
      ) : (
        <FlatList
          data={filtered}
          keyExtractor={item => String(item.ID_DEVOLUCION_VENTA)}
          showsVerticalScrollIndicator={false}
          contentContainerStyle={{ paddingBottom: 24 }}
          renderItem={({ item }) => {
            const cfg = estadoConfig(item.ESTADO_DEVOLUCION_VENTA);
            return (
              <View style={st.card}>
                ...
                <View style={st.cardContent}>
                  <View style={st.cardRow}>
                    <View style={{ flex: 1 }}>
                      <Text style={st.cardTitle}>{item.NUMERO_DEVOLUCION ?? '—'}</Text>
                      <Text style={st.cardSub}>{item.MOTIVO_DEVOLUCION ?? '—'}</Text>
                    </View>
                    <View style={[st.badge, { backgroundColor: cfg.bg }]}>
                      <Text style={[st.badgeText, { color: cfg.color }]}>{cfg.label}</Text>
                    </View>
                  </View>
                  {item.TOTAL_DEVOLUCION_VENTA != null && (
                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 6 }}>
                      <FontAwesome5 name="receipt" size={11} color={Colors.primary} />
                      <Text style={{ fontSize: 15, fontWeight: '700', color: Colors.primary }}>{fmtQ(item.TOTAL_DEVOLUCION_VENTA)}</Text>
                    </View>
                  )}
                  <View style={st.chips}>
                    {!!item.FECHA_DEVOLUCION_VENTA && <Chip icon="calendar-alt" label={fmt(item.FECHA_DEVOLUCION_VENTA)} />}
                    {item.ID_SALIDA_MERCADERIA != null && <Chip icon="box-open" label={`Salida: ${item.ID_SALIDA_MERCADERIA}`} />}
                  </View>
      ...
      <DevolucionVentaModal visible={modalVisible} onClose={() => setModal(false)} item={selected} onSaved={() => { setModal(false); fetch(); }} />
    </View>
  );
}
