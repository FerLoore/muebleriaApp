...
import FacturaVentaDetalleModal from '../../components/factura-venta-detalle/FacturaVentaDetalleModal';
import { Colors } from '../../constants/colors';
import { Layout } from '../../constants/layout';
import { FacturaVentaDetalle, deleteFacturaVentaDetalle, getFacturaVentaDetalles } from '../../services/facturaVentaDetalleService';

function fmtQ(v: number | null) { return v != null ? v.toLocaleString('es-GT', { style: 'currency', currency: 'GTQ' }) : '—'; }

export default function FacturaVentaDetalleScreen() {
  const [data, setData]         = useState<FacturaVentaDetalle[]>([]);
  const [loading, setLoading]   = useState(true);
  const [search, setSearch]     = useState('');
  const [modalVisible, setModal]= useState(false);
  const [selected, setSelected] = useState<FacturaVentaDetalle | null>(null);

  const fetch = async () => {
    setLoading(true);
    try { setData(await getFacturaVentaDetalles()); }
    catch { Toast.show({ type: 'error', text1: 'Error', text2: 'No se pudo cargar los detalles.' }); }
    finally { setLoading(false); }
  };
  useEffect(() => { fetch(); }, []);

  const handleDelete = async (item: FacturaVentaDetalle) => {
    try {
      await deleteFacturaVentaDetalle(item.ID_FACTURA_VENTA_DETALLE);
  ...
  const filtered = data.filter(d =>
    String(d.ID_FACTURA_VENTA_DETALLE).includes(search) ||
    (d.ESTADO_FACTURA_VENTA_DETALLE ?? '').toLowerCase().includes(search.toLowerCase()) ||
    String(d.ID_ARTICULO ?? '').includes(search) ||
    String(d.ID_FACTURA_VENTA ?? '').includes(search)
  );

  const totalItems = data.reduce((a, d) => a + (d.CANTIDAD_FACTURA_VENTA_DETALLE ?? 0), 0);
  const sumaTotal  = data.reduce((a, d) => a + ((d.CANTIDAD_FACTURA_VENTA_DETALLE ?? 0) * (d.PRECIO_UNITARIO_FACTURA_VENTA_DET ?? 0)), 0);

  return (
    <View style={st.container}>
      <View style={st.header}>
        <View>
          <Text style={st.pageTitle}>Detalle Factura Venta</Text>
      ...
      <View style={st.summaryRow}>
        <View style={st.summaryCard}>
          ...
          <Text style={st.summaryLabel}>Registros</Text>
        </View>
        <View style={st.summaryCard}>
          <FontAwesome5 name="boxes" size={13} color="#7C3AED" />
          <Text style={[st.summaryValue, { color: '#7C3AED' }]}>{totalItems}</Text>
          <Text style={st.summaryLabel}>Unidades</Text>
        </View>
        <View style={[st.summaryCard, { flex: 2 }]}>
          <FontAwesome5 name="dollar-sign" size={13} color="#059669" />
          <Text style={[st.summaryValue, { color: '#059669', fontSize: 12 }]}>{fmtQ(sumaTotal)}</Text>
          <Text style={st.summaryLabel}>Total lineas</Text>
        </View>
      </View>
      ...
      {loading ? (
        ...
        <View style={st.empty}><FontAwesome5 name="file-invoice" size={36} color={Colors.textMuted} /><Text style={st.emptyText}>Sin resultados</Text></View>
      ) : (
        <FlatList
          data={filtered}
          keyExtractor={item => String(item.ID_FACTURA_VENTA_DETALLE)}
          showsVerticalScrollIndicator={false}
          contentContainerStyle={{ paddingBottom: 24 }}
          renderItem={({ item }) => {
            const subtotal = (item.CANTIDAD_FACTURA_VENTA_DETALLE ?? 0) * (item.PRECIO_UNITARIO_FACTURA_VENTA_DET ?? 0);
            return (
              <View style={st.card}>
                ...
                <View style={st.cardContent}>
                  <View style={st.cardRow}>
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={st.cardSub}>Factura ID: {item.ID_FACTURA_VENTA ?? '—'} · Detalle #{item.ID_FACTURA_VENTA_DETALLE}</Text>
                    </View>
                    <View style={[st.badge, { backgroundColor: '#ECFDF5' }]}>
                      <Text style={[st.badgeText, { color: '#059669' }]}>{item.ESTADO_FACTURA_VENTA_DETALLE ?? '—'}</Text>
                    </View>
                  </View>
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 6 }}>
                    <FontAwesome5 name="receipt" size={11} color={Colors.primary} />
                    <Text style={{ fontSize: 14, fontWeight: '700', color: Colors.primary }}>{fmtQ(subtotal)}</Text>
                  </View>
                  <View style={st.chips}>
                    {item.CANTIDAD_FACTURA_VENTA_DETALLE != null &&
                      <Chip icon="boxes" label={`Cant: ${item.CANTIDAD_FACTURA_VENTA_DETALLE}`} />}
                    {item.PRECIO_UNITARIO_FACTURA_VENTA_DET != null &&
                      <Chip icon="tag" label={`P.U: ${fmtQ(item.PRECIO_UNITARIO_FACTURA_VENTA_DET)}`} />}
                  </View>
      ...
      <FacturaVentaDetalleModal visible={modalVisible} onClose={() => setModal(false)} item={selected} onSaved={() => { setModal(false); fetch(); }} />
    </View>
  );
}
