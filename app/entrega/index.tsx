...
import EntregaModal from '../../components/entrega/EntregaModal';
import { Colors } from '../../constants/colors';
import { Layout } from '../../constants/layout';
import { Entrega, deleteEntrega, getEntregas } from '../../services/entregaService';
...
export default function EntregaScreen() {
  ...
  const fetch = async () => {
    setLoading(true);
    try { setData(await getEntregas()); }
    catch { Toast.show({ type: 'error', text1: 'Error', text2: 'No se pudo cargar entregas.' }); }
    finally { setLoading(false); }
  };

  useEffect(() => { fetch(); }, []);

  const handleDelete = async (item: Entrega) => {
    try {
      await deleteEntrega(item.ID_ENTREGA);
      Toast.show({ type: 'success', text1: 'Eliminado', text2: 'Entrega eliminada.' });
      fetch();
    } catch { Toast.show({ type: 'error', text1: 'Error', text2: 'No se pudo eliminar.' }); }
  };

  const filtered = data.filter(d =>
    String(d.ID_ENTREGA).includes(search) ||
    (d.NOMBRE_RECIBE_ENTREGA ?? '').toLowerCase().includes(search.toLowerCase()) ||
    (d.APELLIDOS_RECIBE_ENTREGA ?? '').toLowerCase().includes(search.toLowerCase()) ||
    (d.DPI_RECIBE_ENTREGA ?? '').includes(search) ||
    (d.UBICACION_ENTREGA ?? '').toLowerCase().includes(search.toLowerCase())
  );

  const openCreate = () => { setSelected(null); setModal(true); };
  const openEdit   = (item: Entrega) => { setSelected(item); setModal(true); };

  return (
    <View style={st.container}>
      <View style={st.header}>
        <View>
          <Text style={st.pageTitle}>Entregas</Text>
      ...
      <View style={st.summaryRow}>
        <View style={st.summaryCard}>
          <FontAwesome5 name="box" size={13} color={Colors.primary} />
          <Text style={[st.summaryValue, { color: Colors.primary }]}>{data.length}</Text>
          <Text style={st.summaryLabel}>Total</Text>
        </View>
        <View style={st.summaryCard}>
          <FontAwesome5 name="calendar-check" size={13} color="#059669" />
          <Text style={[st.summaryValue, { color: '#059669' }]}>
            {data.filter(d => !!d.FECHA_ENTREGA).length}
          </Text>
          <Text style={st.summaryLabel}>Con Fecha</Text>
        </View>
        <View style={st.summaryCard}>
          <FontAwesome5 name="map-marker-alt" size={13} color="#3B82F6" />
          <Text style={[st.summaryValue, { color: '#3B82F6' }]}>
            {data.filter(d => !!d.UBICACION_ENTREGA).length}
          </Text>
          <Text style={st.summaryLabel}>Con Ubicación</Text>
        </View>
      </View>

      <View style={st.searchWrapper}>
        <FontAwesome5 name="search" size={12} color={Colors.textMuted} />
        <TextInput style={st.searchInput} placeholder="Buscar por nombre, DPI o ubicación…" placeholderTextColor={Colors.textMuted} value={search} onChangeText={setSearch} />
      ...
      {loading ? (
        ...
        <FlatList
          data={filtered}
          keyExtractor={item => String(item.ID_ENTREGA)}
              ...
              <View style={st.cardContent}>
                <View style={st.cardRow}>
                  <View style={st.avatar}>
                    <Text style={st.avatarText}>{(item.NOMBRE_RECIBE_ENTREGA ?? '?')[0].toUpperCase()}</Text>
                  </View>
                  <View style={{ flex: 1, marginLeft: 10 }}>
                    <Text style={st.cardTitle} numberOfLines={1}>
                      {item.NOMBRE_RECIBE_ENTREGA ?? '—'} {item.APELLIDOS_RECIBE_ENTREGA ?? ''}
                    </Text>
                    <Text style={st.cardSub}>DPI: {item.DPI_RECIBE_ENTREGA ?? '—'} · ID: {item.ID_ENTREGA}</Text>
                  </View>
                </View>
                <View style={st.chips}>
                  {!!item.FECHA_ENTREGA    && <Chip icon="calendar-alt" label={item.FECHA_ENTREGA} />}
                  {!!item.UBICACION_ENTREGA && <Chip icon="map-marker-alt" label={item.UBICACION_ENTREGA} />}
                  {item.ID_DESPACHO_DETALLE != null && <Chip icon="shipping-fast" label={`Despacho: ${item.ID_DESPACHO_DETALLE}`} />}
                </View>
                {!!item.OBSERVACION_ENTREGA && (
                  <Text style={st.obs} numberOfLines={2}>{item.OBSERVACION_ENTREGA}</Text>
                )}
      ...
      <EntregaModal visible={modalVisible} onClose={() => setModal(false)} item={selected} onSaved={() => { setModal(false); fetch(); }} />
    </View>
  );
}
