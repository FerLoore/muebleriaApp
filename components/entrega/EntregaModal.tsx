...
import { getDespachoDetalles } from '../../services/despachoDetalleService';
...
const EMPTY: Record<string, string> = {
  ...
  OBSERVACION_ENTREGA: '', UBICACION_ENTREGA: '',
};
...
export default function EntregaModal({ visible, onClose, item, onSaved }: Props) {
  ...
  const [despachos, setDespachos]         = useState<DropdownOption[]>([]);
  ...
  useEffect(() => {
    ...
    setLoadingOpts(true);
    getDespachoDetalles()
      .then(data => setDespachos(data.map(d => ({
        value: d.ID_ORDEN_DESPACHO,    // PK compuesta: usamos el ID de orden como value
        label: `Orden ${d.ID_ORDEN_DESPACHO} — Sec. ${d.SECUENCIA_ENTREGA_DESPACHO_DETALLE} (${d.ESTADO_DESPACHO_DETALLE ?? '—'})`,
      }))))
      .catch(() => Toast.show({ type: 'error', text1: 'Error', text2: 'No se pudieron cargar despachos.' }))
  ...
  useEffect(() => {
    if (item) {
      ...
      setIdDespachoDetalle(item.ID_DESPACHO_DETALLE);
    } else {
      setForm(EMPTY);
  ...
  const handleSave = async () => {
    ...
    const payload: Omit<Entrega, 'ID_ENTREGA'> = {
      ...
      NOMBRE_RECIBE_ENTREGA:    form.NOMBRE_RECIBE_ENTREGA || null,
      APELLIDOS_RECIBE_ENTREGA: form.APELLIDOS_RECIBE_ENTREGA || null,
      DPI_RECIBE_ENTREGA:       form.DPI_RECIBE_ENTREGA || null,
      FIRMA_RECIBE_ENTREGA:     form.FIRMA_RECIBE_ENTREGA || null,
      FOTO_ENTREGA:             form.FOTO_ENTREGA || null,
      OBSERVACION_ENTREGA:      form.OBSERVACION_ENTREGA || null,
      UBICACION_ENTREGA:        form.UBICACION_ENTREGA || null,
  ...
  return (
    <Drawer visible={visible} onClose={onClose} title={item ? 'Editar Entrega' : 'Nueva Entrega'}>
      <ScrollView style={s.scroll} contentContainerStyle={s.content} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        ...
        <Sec label="Despacho asociado" />
        <DropdownSelect
          label="Despacho Detalle"
          value={idDespachoDetalle}
          onChange={setIdDespachoDetalle}
          options={despachos}
          loading={loadingOpts}
      ...
      <View style={s.footer}>
        ...
        <TouchableOpacity style={[s.btnSave, saving && { opacity: 0.6 }]} onPress={handleSave} disabled={saving}>
          {saving ? <ActivityIndicator color="#fff" size="small" /> : <><FontAwesome5 name="save" size={13} color="#fff" /><Text style={s.btnSaveText}>{item ? 'Guardar cambios' : 'Crear entrega'}</Text></>}
        </TouchableOpacity>
      </View>
    </Drawer>
  );
